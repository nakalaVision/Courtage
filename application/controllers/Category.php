...
class Category extends CI_Controller{
    ...
    {   
		...
		$this->form_validation->set_rules('libelleCategorie','LibelleCategorie','required');
		
		if($this->form_validation->run())     
        {   
            $params = array(
				'libelleCategorie' => $this->input->post('libelleCategorie'),
            );
     ...
     * Editing a category
     */
    function edit($idCategories)
    {   
        // check if the category exists before trying to edit it
        $data['category'] = $this->Category_model->get_category($idCategories);
        
        if(isset($data['category']['idCategories']))
        {
            $this->load->library('form_validation');
            ...
            {   
                ...
                $this->Category_model->update_category($idCategories,$params);            
                redirect('category/index');
            }
            else
            {
     ...
     * Deleting category
     */
    function remove($idCategories)
    {
        $category = $this->Category_model->get_category($idCategories);

        // check if the category exists before trying to delete it
        if(isset($category['idCategories']))
        {
            $this->Category_model->delete_category($idCategories);
            redirect('category/index');
        }
        else
